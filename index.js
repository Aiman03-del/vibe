...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const imageUrl = `http://localhost:4000/${req.files.image[0].path.replace(
        /\\/g,
        "/"
      )}`;
      const fileUrl = `http://localhost:4000/${req.files.file[0].path.replace(
        /\\/g,
        "/"
      )}`;
...
app.listen(PORT, () => {
  console.log(`Server is running at http://localhost:${PORT}/songs`);
});
